...
$(function() {
  ...
  prestashop.on('updatedProduct', (event) => {
    const productQuantity = typeof document.getElementById('stripe_product_quantity').value !== 'undefined' ? document.getElementById('stripe_product_quantity').value : '0';
    const quantity = typeof document.getElementById('quantity_wanted').value !== 'undefined' ? document.getElementById('quantity_wanted').value : '';
  ...
  prestashop.on('updateCart', (args) => {
    if (args.resp && (args.resp.errors !== '' || args.resp.cart.products.length === 0)) {
      if (expressCheckoutElement) {
        expressCheckoutElement.unmount();
      }
    } else {
...
async function initialize() {
  ...
  {
    const productQuantity = typeof document.getElementById('stripe_product_quantity') !== 'undefined' ? document.getElementById('stripe_product_quantity').value : '0';
    $quantity = typeof document.getElementById('quantity_wanted') !== 'undefined' ? document.getElementById('quantity_wanted').value : '';
    if (parseInt(productQuantity) <= 0 && stripe_product_out_of_stock !== 1) {
      return;
    }
    $pageName = 'product';

    amount = stripe_express_amount * $quantity;
  ...
  expressCheckoutElement.on('click', (event) => {
    ...
    if(document.body.id === 'product')
    {
      let productQuantity = typeof document.getElementById('quantity_wanted').value !== 'undefined' ? document.getElementById('quantity_wanted').value : '';

      $quantity = productQuantity;
      let amountProductString = document.querySelector('.current-price span').innerHTML;
      let findAmount = /[0-9](.+)/.exec(amountProductString)[0];
      findAmount = findAmount.replace(',', '.');
      let productAmount = parseInt((parseFloat(findAmount) * 100).toFixed());
      amount = productAmount * productQuantity;

      elements.update({amount: amount});
    }
    if (document.body.id === 'cart') {
      let cartTotalString = document.querySelector('.cart-total .value').innerHTML;
      let str = /[0-9](.+)/.exec(cartTotalString)[0];
      str = str.replace(',', '.');
      let productTotalAmount = parseInt((parseFloat(str) * 100).toFixed());
      ...
      if (shippingElement) {
        ...
        if (shippingMatch && shippingMatch[0]) {
          ...
          shippingTotal = parseInt((parseFloat(shipping) * 100).toFixed());
        }
      }
